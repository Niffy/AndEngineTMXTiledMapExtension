...
/**
 * Currently only Isometric tile objects is supported. extends
 * {@link TMXLayerHighPerformanceSpriteBatch} since its pretty similar stuff.
 * This process a given object group, which the user believes are tiles, into a
 * {@link TMXLayerHighPerformanceSpriteBatch}. Be warned when operating on these
 * layers, always check for null values first.
 * 
 * @author Paul Robinson
 * 
 */
public class TMXLayerObjectTilesHighPerformanceSpriteBatch extends TMXLayerHighPerformanceSpriteBatch {
	...
	/**
	 * Create a new TMXLayerObjectTiles based on the {@link TMXLayer} class This
	 * will take in an {@link TMXObjectGroup} and produce a replica
	 * {@link TMXLayer} to be drawn.
	 * 
	 * @param pTMXTiledMap
	 *            {@link TMXTiledMap}
	 * @param pAttributes
	 *            For this just pass null as it is not used.
	 * @param pVertexBufferObjectManager
	 *            {@link VertexBufferObjectManager}
	 */
	public TMXLayerObjectTilesHighPerformanceSpriteBatch(TMXTiledMap pTMXTiledMap, Attributes pAttributes,
	...
	@Override
	public void initializeTMXTileFromXML(Attributes pAttributes, ITMXTilePropertiesListener pTMXTilePropertyListener) {
		return; // Do nothing
	}

	@Override
	public void initializeTMXTilesFromDataString(String pDataString, String pDataEncoding, String pDataCompression,
			ITMXTilePropertiesListener pTMXTilePropertyListener) throws IOException, IllegalArgumentException {
		return; // Do nothing
	}
	...
	private void processTMXObject() {
		if (this.mTMXTiledMap.getOrientation().equals(TMXConstants.TAG_MAP_ATTRIBUTE_ORIENTATION_VALUE_ISOMETRIC)) {
			this.processIsometricTMXObject();
				...
				TMXConstants.TAG_MAP_ATTRIBUTE_ORIENTATION_VALUE_ORTHOGONAL)) {
			Log.w(TAG, "Currently Orthographic is not supported for drawing tile objects");
		} else {
			Log.w(TAG, String.format(
					"processTMXObject - Orientation not supported: '%s'. Will use normal Orthogonal draw method",
	...
	private void processIsometricTMXObject() {
		...
		for (TMXObject tmxObject : objects) {
			...
			if (columnPixel <= 0 || rowPixel <= 0 || columnPixel > maxColumnPixel || rowPixel > maxRowPixel) {
				// We've got a tile outside the map, we won't draw it so skip
				// it.
				continue;
			}
			...
			if (tmxTileTextureRegion != null) {
				...
				if (this.mTexture == null) {
					...
					if (this.mTexture != tmxTileTextureRegion.getTexture()) {
						throw new AndEngineRuntimeException("All TMXTiles in a TMXObjetTileLayer (" + this.getName()
			...
			TMXTile tmxTile = new TMXTile(this.mTMXTiledMap.getOrientation(), pGlobalTileID, pZindex, column, row,
			...
			if (pGlobalTileID != 0) {
				...
				// Before we were drawing to the map tile size, not the tileset
				// size
				this.drawWithoutChecks(tmxTileTextureRegion, tmxTile.getTileX(), tmxTile.getTileY(),
						offset_tilesize[2], offset_tilesize[3], Color.WHITE_ABGR_PACKED_FLOAT);
				this.submit(); // TODO Doesn't need to be called here, but
								// should rather be called in a "init" step,
								// when parsing the XML is complete.
